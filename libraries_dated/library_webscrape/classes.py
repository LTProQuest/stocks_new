...
class webscraper:
    ...
    def get_soup(self, url, find_in_soup=None):
        
        r = webscraper.s.get(url, cookies=webscraper.cj)
        plain_text = r.text    
    ...
    def download_file_from_url(self, download_url, file_directory=None):
        if self.debug == True:
        ...
        def get_filename_from_cd(cd):     
            ...
            if len(fname) == 0:
                return None
            return fname[0]
        response = webscraper.s.get(download_url, cookies=webscraper.cj ,allow_redirects=True)
    ...
    def api_post_payload(self, api_url, payload):       
